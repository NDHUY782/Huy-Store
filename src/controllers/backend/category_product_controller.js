...
const uploadAvatar = fileHelpers.upload(
  "avatar",
  `${__path_public}uploads/category_products/`
);
...
module.exports = {
  getlist: async (req, res, next) => {
    ...
    let sortField = paramsHelpers.getParam(
      req.session,
      "sortField",
      "ordering"
    );
    ...
    let { data } = await CategoryProductService.getAll({
      pagination,
      condition,
      sort,
    });
  ...
  getSort: async (req, res, next) => {
    req.session.sortField = paramsHelpers.getParam(
      req.params,
      "sort_field",
      "ordering"
    );
    req.session.sortType = paramsHelpers.getParam(
      req.params,
      "sort_type",
      "asc"
    );
      ...
      if (err) {
        ...
        if (typeof item !== "undefined" && item.id !== "") {
          ...
          if (req.files == undefined || req.files.length == 0) {
            ...
            fileHelpers.remove(
              "src/public/uploads/category_products/",
              item.image_old
            );
          }
    ...
    if (action === "delete") {
      if (Array.isArray(id)) {
        for (let index = 0; index < id.length; index++) {
          let { avatar } = await CategoryProductService.findAvatar({
            id: id[index],
          });
          fileHelpers.remove("src/public/uploads/category_products/", avatar);
        }
      ...
      let { deletedCount } = await CategoryProductService.ChangeDeleteMultiple({
        id,
      });
      req.flash(
        "success",
        util.format(notify.DELETE_MULTI_SUCCESS, deletedCount)
      );
      res.redirect(`${linkPrefix}`);
    } else {
      let data = {
      ...
      let { modifiedCount } = await CategoryProductService.ChangeStatusMultiple(
        { id, data }
      );
      req.flash(
        "success",
        util.format(notify.CHANGE_STATUS_MULTI_SUCCESS, modifiedCount)
      );
      res.redirect(`${linkPrefix}`);
    }
  },
