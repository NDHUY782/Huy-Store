...
module.exports = {
  getlist: async (req, res, next) => {
    ...
    let { data } = await CategoryAccountService.getAll({
      pagination,
      condition,
      sort,
    });
  ...
  changeMultipleAction: async (req, res, next) => {
    ...
    if (action === "delete") {
      let { deletedCount } = await CategoryAccountService.ChangeDeleteMultiple({
        arrId,
      });
      req.flash(
        "success",
      ...
      let { modifiedCount } = await CategoryAccountService.ChangeStatusMultiple(
        { arrId, action }
      );
      req.flash(
        "success",
